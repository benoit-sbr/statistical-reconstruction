...
A = np.genfromtxt('gene_2624_nettoye.txt', missing_values = 'NA', filling_values = np.nan, skip_header = 1)
...
#            somme += (np.exp(delta*temps[j+1])-np.exp(delta*temps[j])) * eta[j] / (gamma + eta[j])
            somme += (np.exp(delta*temps[j+1])-np.exp(delta*temps[j])) * coef[j]
        ### fin de ce calcul
        # calcul de mu_c(t)
...
#    alphaA, betaA, gammaA, deltaA, mu0A, alphaM, betaM, gammaM, deltaM, mu0M, sigma = x
    alphaA, betaA, gammaA, deltaA, mu0A, sigma = x
#    for c in range(C): # entrée dans la boucle sur les conditions
    for c in range(C-1): # entrée dans la boucle sur les conditions
        eta = etaAM[c, :]
...
x0 = np.asarray((10, 10, 100, 1, mu0, 1)) # Initial guess
...
alphaA, betaA, gammaA, deltaA, mu0A, sigma = res1.x

## théorie # à faire : améliorer pour passer alpha, beta, gamma, delta, mu0 en paramètres
alpha, beta, gamma, delta, mu0 = alphaA, betaA, gammaA, deltaA, mu0A
